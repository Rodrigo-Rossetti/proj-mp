...
export const downloadCSV = (data: Transaction[]): void => {
  ...
  link.setAttribute('download', `mePouPay-relatorio-${new Date().toISOString().slice(0, 10)}.csv`);
...
export const downloadPDF = (data: Transaction[]): void => {
  try {
    ...
    // Save the PDF
    const fileName = `mePouPay-relatorio-${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
